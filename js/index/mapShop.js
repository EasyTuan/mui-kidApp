...
gdMap = {
		...
		request('/Store/getStoreDetail', {
			...
			var marker = new AMap.Marker({
				...
				"content": "<div class='experience-image'>" +
					"<span class='map-icon-url' data_id='" + StoreId + "' style='position:absolute;border-radius:6px ;width:90px;height:20px;text-align: center;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;font-size:12px;color:#666;left:50%;bottom:50px;margin-left:-45px;border-radio:25%;background: #fff;'>" + StoreName + "</span>" +
	...
	setMarkersBySearchResult: function(dataParm) {
		//点聚合对象(cluster)AMap.MarkerClusterer
		//API http://lbs.amap.com/api/javascript-api/reference/plugin/#AMap.MarkerClusterer
		console.log(mklog(JSON.stringify(dataParm)) + '进入搜索结果打点')
		mapObj.remove(markers);
		markers = []; //fix 重复打点
		request('/Store/searchStoreList', {
			...
			console.log(mklog() + '【debug】ajax获取到地图上的点集合:' + r.data.length)

			if(markers.length) {
				console.log(mklog() + '【debug】点数量:' + markers.length)
				gdMap.addClusterToMap(markers) //执行一次
			}
		})
	...
	addClusterToMap: function(markers) {
		//		console.log(mklog() + '【debug】addClusterToMap(' + JSON.stringify(markers) + ')')
		//		var cluster = null;
		var mkStyles = [{
		...
		mapObj.plugin(["AMap.MarkerClusterer"], function() {
			//			console.log(mklog() + '【debug】AMap.MarkerClusterer(mapObj,' + JSON.stringify(markers) + ')')
			cluster = new AMap.MarkerClusterer(mapObj, markers, {
				styles: mkStyles
			})
			//			console.log(mklog() + '【debug】cluster=' + cluster)
	...
	removeCityMarkers: function(markers) {
		console.log('清除地图上的' + markers.length + '个点')
		//		mapObj.remove(markers);
	}
